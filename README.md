...
| **123 / カナ キー** | 左列の下から2番目。通常は `123`（数字レイアウトへ、押すと `かな` に変わって戻れる）、変換中は `カナ`（未確定文字列をカタカナに変換）。上フリックで他の IME へ切り替え |
...
JVM 単体テストが 41 件ある（`./gradlew testDebugUnitTest`）。
